...
class EditPelajarPage extends StatefulWidget {
  final int mahasiswaId;

  const EditPelajarPage({super.key, required this.mahasiswaId});

  @override
  State<EditPelajarPage> createState() => _EditPelajarPageState();
}

class _EditPelajarPageState extends State<EditPelajarPage> {
  ...
  void initState() {
    ...
    _fetchKelasAndNip();
    fetchJurusan();
    _fetchMahasiswaDetail(); // ← ini penting
  }

  Future<void> _fetchMahasiswaDetail() async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token');

    final uri = Uri.parse(
      'http://192.168.18.13:8080/api/admin/mahasiswa/${widget.mahasiswaId}',
    );
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        _selectedJurusan = data['jurusan']?['nama_jurusan'];
        _selectedKelasId = data['kelas_id'];
        _gender = data['jenis_kelamin'] ?? 'Laki-laki';
      });
    } else {
      // Jika gagal, tampilkan pesan error
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gagal memuat data mahasiswa')),
      );
    }
  }
  ...
  void _handleUpdate() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final uri = Uri.parse(
        'http://192.168.18.13:8080/api/admin/admin/mahasiswa/${widget.mahasiswaId}',
      );
      ...
      final response = await http.put(
        ...
        body: jsonEncode({
          ...
          'password_confirmation':
              _password_confirmationController.text.isEmpty
                  ? null
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(err['message'] ?? 'Gagal memperbarui data')),
        );
      }
    }
  }
